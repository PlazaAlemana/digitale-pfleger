...
class PDFMistralAgent:
    ...
    def _extract_text_with_mistral_ocr(self, pdf_path: str) -> str:
        """
        Extract text from PDF using Mistral OCR API with CORRECT format.
        ...
        """
        print("   🔍 Using Mistral OCR to extract text...")
        print("   ⏳ This may take a moment (processing with AI)...")
        ...
        try:
            ...
            # ✅ CORRECT API FORMAT (per official Mistral docs)
            ocr_response = self.client.ocr.process(
                model="mistral-ocr-latest",
                document={
                    "type": "document_url",  # ✅ Use "document_url" not "document_base64"
                    "document_url": f"data:application/pdf;base64,{base64_pdf}"  # ✅ Format as data URI
                },
            ...
            if not markdown_text or not markdown_text.strip():
                raise Exception("No text extracted from OCR response")

            # Clean up markdown while preserving structure
            markdown_text = re.sub(r'\n\s*\n\s*\n+', '\n\n', markdown_text)
            markdown_text = re.sub(r'[ \t]+', ' ', markdown_text)

            print("   ✅ Mistral OCR extraction completed successfully!")
            ...
            if "validation" in str(e).lower():
                print("   💡 API format issue. Check Mistral AI documentation.")
            raise Exception(f"Mistral OCR extraction failed: {str(e)}")

    def load_pdf(self, pdf_path: str) -> bool:
        """
        Load and process a PDF file with validation and enhanced text extraction.
        ...
        try:
            ...
            # Try enhanced PyPDF2 extraction first
            self.pdf_content = self._extract_pdf_text_enhanced(pdf_path)

            # If no text extracted, try OCR
            if not self.pdf_content.strip():
                print("   ⚠️  No text extracted with standard method")
                print("   🔄 Attempting Mistral OCR extraction...")

                try:
                    self.pdf_content = self._extract_text_with_mistral_ocr(pdf_path)
                except Exception as ocr_error:
                    print(f"   ❌ Mistral OCR failed: {str(ocr_error)}")
                    print("   💡 This PDF appears to be image-based or scanned.")
                    print("   💡 Try converting it to a searchable PDF using:")
                    print("      - Adobe Acrobat")
                    print("      - Online OCR tools")
                    print("      - Or use a text-based PDF")
                    return False
